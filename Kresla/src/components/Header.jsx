          ...
          <div className="container mx-auto px-4 py-4 space-y-2">
            <Link
              ...
              className={`block px-4 py-3 rounded-lg transition-colors font-medium ${
                isActive('/') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-indigo-50'
              }`}
            ...
            <Link
              to="/catalog"
              className={`block px-4 py-3 rounded-lg transition-colors font-medium ${
                isActive('/catalog') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-indigo-50'
              }`}
              onClick={() => setIsMenuOpen(false)}
            >
              Каталог
            ...
            <div className="px-4 py-2">
              <p className="text-gray-400 text-xs uppercase tracking-wider font-semibold mb-2">О компании</p>
              <div className="space-y-1 ml-4 border-l-2 border-indigo-100 pl-4">
                {aboutPages.map((page) => (
                  <Link
                    key={page.path}
                    to={page.path}
                    className={`block py-2 text-gray-700 hover:text-indigo-600 transition-colors ${
                      isActive(page.path) ? 'text-indigo-600' : ''
                    }`}
                    onClick={() => setIsMenuOpen(false)}
                  >
                    {page.label}
            ...
            <Link
              to="/advantages"
              className={`block px-4 py-3 rounded-lg transition-colors font-medium ${
                isActive('/advantages') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-indigo-50'
              }`}
              onClick={() => setIsMenuOpen(false)}
            >
              Преимущества
            ...
            <Link
              to="/testimonials"
              className={`block px-4 py-3 rounded-lg transition-colors font-medium ${
                isActive('/testimonials') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-indigo-50'
              }`}
              onClick={() => setIsMenuOpen(false)}
            >
              Отзывы
            ...
            <Link
              to="/contacts"
              className={`block px-4 py-3 rounded-lg transition-colors font-medium ${
                isActive('/contacts') ? 'bg-indigo-50 text-indigo-600' : 'text-gray-700 hover:bg-indigo-50'
              }`}
              onClick={() => setIsMenuOpen(false)}
            >
              Контакты
